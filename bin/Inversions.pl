#!/usr/bin/perl
#$number_inversions=20;
#$minimum_lines_to_skip = 0;
chomp($ARGV[0]);
chomp($ARGV[1]);
chomp($ARGV[2]);
$number_inversions=$ARGV[2];
my $bases_per_line = -1;
...
#die "$max_lines $minimum_lines_to_skip\n";
#chomp $max_lines;	
...
my @random_numbers = map {random_int_between($min_base,$max_base)}(1..$number_inversions);#Choose a series of positions to mutate
@random_inversions=sort {$a <=> $b} @random_numbers;
###################################
###################################
#### Now add Large inversions ######
###################################
###################################
...
open (OUT,">>$ARGV[1]")||die "Can't write to output file!\n";
	...
	$complete=0;
	my $min_inversion_size=1000;
	my $max_inversion_size=50000;

foreach $random_number(@random_inversions){
		chomp $random_number;
		...
		$line =`awk -v stop=$temp '(NR==stop)' $ARGV[0]`;
			if ($line !~ /N/){
			chomp $line;
			$inversion_length = random_int_between($min_inversion_size,$max_inversion_size);
			my $position=$random_number;					# Adjust position
			my $ref_base=substr($line,$target_position-1,1);  # Get reference Base
			my $inversion_length=$inversion_length+1;
			my $end=$inversion_length+$position;
			if($position =~ m/\d+/ && $ref_base =~ m/A|T|G|C/i)
			{ 
				print OUT "$chrid\t$position\t.\t$ref_base\t<INV>\t.\t.\tSVTYPE=INV;END=$end;SVLEN=$inversion_length\n";
				$complete++;
			}
			next}	
		}
print "Number Inversions complete = $complete \n";
close OUT;
