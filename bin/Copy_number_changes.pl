#!/usr/bin/perl
#$number_duplications=500;
#$minimum_lines_to_skip = 200000;
chomp($ARGV[0]);
chomp($ARGV[1]);
chomp($ARGV[2]);
$number_duplications=$ARGV[2];
my $bases_per_line = -1;
...
### Set initial Parameters
  ...
  my @random_numbers = map {random_int_between($min_base,$max_base)}(1..$number_duplications);#Choose a series of positions to mutate
  @random_duplications=sort {$a <=> $b} @random_numbers;
###################################
###################################
#### Now add Tandem Duplications ##
###################################
###################################
...
open (OUT,">>$ARGV[1]")||die "Can't write to output file!\n";
	...
	$complete=0;
	my $min_duplication_size=1000;
	my $max_duplication_size=50000;

foreach $random_number(@random_duplications){
		chomp $random_number;
		...
		$line =`awk -v stop=$temp '(NR==stop)' $ARGV[0]`;
			if ($line !~ /N/){
			chomp $line;
			$duplication_length = random_int_between($min_duplication_size,$max_duplication_size);
			my $position=$random_number;					# Adjust position
			my $ref_base=substr($line,$target_position-1,1);  # Get reference Base
			my $duplication_length=$duplication_length+1;
			my $end=$duplication_length+$position;
			#$ref_base =~ s/[\t,\n,\t, ]//g;
			if($position =~ m/\d+/ && $ref_base =~ m/A|T|G|C/i)
			{
			print OUT "$chrid\t$position\t.\t$ref_base\t<DUP:TANDEM>\t.\t.\tSVTYPE=DUP;END=$end;SVLEN=$duplication_length\n";
			$complete++;
			}
			next}	
		}
print"Number Novel duplications complete = $complete \n";
close OUT;
